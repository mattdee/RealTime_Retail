...
grant all on dw_demo.* to retail@127.0.0.1;
...
LOAD DATA INFILE '/tmp/states.csv'  ignore INTO TABLE states  FIELDS TERMINATED BY ','  OPTIONALLY ENCLOSED BY '"' LINES TERMINATED BY '\n' ignore 1 lines (state,abbreviation);

drop table if exists zipcode;
create reference table zipcode
  (
    zip int primary key,
    type varchar(100),
  ...
  ;
/* wget https://www.dropbox.com/s/68q6zbdukbhpb1c/zip_code_database.csv */
LOAD DATA INFILE '/tmp/zip_code_database.csv'  ignore INTO TABLE zipcode  FIELDS TERMINATED BY ','  OPTIONALLY ENCLOSED BY '"' LINES TERMINATED BY '\n' ignore 1 lines 
